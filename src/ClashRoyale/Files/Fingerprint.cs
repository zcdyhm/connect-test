            ...
            try
            {
                if (File.Exists(Path))
                {
                    ...
                    var json = JObject.Parse(Json);
                    {
                        ...
                        Logger.Log($"Fingerprint [v{GetVersion}] 已加载.",
                            GetType());
                    }
                }
                else
                {
                    Console.WriteLine("指纹无法加载，文件不存在.");
                    Program.Exit();
                }
            }
            catch (Exception)
            {
                Console.WriteLine("指纹加载失败.");
                Program.Exit();
            }
        }
    ...
    public class Asset
    {
        ...
        public async Task<bool> HasFileChanged()
        {
            ...
            switch (expression)
            {
                ...
                default:
                {
                    Logger.Log($"未知的文件表达式 {expression}", GetType(), ErrorLevel.Warning);
                    break;
                }
            }
